...
const paymentForm = new PaymentForm(cloneTemplate < HTMLFormElement > (paymentTemplate), events);
...
events.on('contacts:submit', () => {
	api.orderResult(appState.order).then((res) => {
...
events.on(/(^order|^contacts)\..*:change/, (data: {
	field: keyof IContactsForm | keyof IPaymentForm;value: string
}) => {
...
api.getProductList().then(appState.setCatalog.bind(appState)).catch((err) => {
	console.error(err);
});